(function($) {
    $.fn.listswap = function(options) {		
		...
		var controls = '<div class="listbox_controls">' + 
							'<ul>' +
					   			'<li class="add button"><span class="arrow"></span><span class="label">' + settings.label_add + '</span></li>' +
					   			'<li class="remove button"><span class="arrow"></span><span class="label">' + settings.label_remove + '</span></li>' +
							'</ul>' +
					   '</div>';
		...
		if (source_search) {
			var search_source_selector = div_id + ' .source_wrapper';
			search_filter(search_source_selector);
			reset_search_input(search_source_selector, source_select_id);
			clear_button(search_source_selector);
		}
		 
		if (destination_search) {
			var search_destination_selector = div_id + ' .destination_wrapper';
			search_filter(search_destination_selector);
			reset_search_input(search_destination_selector, destination_select_id);
			clear_button(search_destination_selector);
		}
		
		refresh_list();

		function init_selects() {
            var $select_source = $( "#" + source_select_id );
            var $select_destination = $( "#" + destination_select_id );
            $select_source.attr( "multiple", true );
            $select_source.children().each( function(){
                $(this).attr( "selected", true );
            });
            $select_destination.attr( "multiple", true );
            $select_destination.children().each( function(){
                $(this).attr( "selected", true );
            });
        }
		...
		function add_remove_handler(target, destination, select_1, select_2) {
            ...
            var $optionToAppend  = $("<option></option>").attr("value",data_value).text(text).attr( "selected", true );
            $(div_id + " select#" + select_2).append( $optionToAppend );
			target.remove();
		}
